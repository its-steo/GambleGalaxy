        ...
        if (checkoutRequestId) {
            pollingInterval = setInterval(async () => {
                try {
                    ...
                    if (res.status === 200 && data.status !== "pending") {
                        ...
                        clearTimeout(timeout!);
                        setCheckoutRequestId(null);
                        toast.error("Transaction Status Error", {
                            description: data.message || "Failed to fetch transaction status.",
                            className: "bg-red-500/90 text-white border-red-400",
                        });
                    }
                } catch (error) {
                    ...
                    toast.error("Network Error", {
                        description: "Could not fetch transaction status. Please check your connection.",
                        className: "bg-red-500/90 text-white border-red-400",
                    });
                }
            }, 10000);
            ...
            timeout = setTimeout(() => {
                ...
                toast.error("Transaction Timeout", {
                    description: "The deposit transaction timed out. Please try again.",
                    className: "bg-red-500/90 text-white border-red-400",
                });
            }, 120000);
        }
