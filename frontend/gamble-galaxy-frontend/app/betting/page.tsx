...
// Type for BigGameImage from API
...
const formatOdds = (value: unknown): string => {
  const num = Number(value)
...
const BettingPage = () => {
  ...
  // Load matches, bet history, and big games only when authenticated and not loading
  useEffect(() => {
    if (!authLoading && isAuthenticated) {
      loadMatches()
      loadBetHistory()
  ...
  const fetchBigGames = async () => {
    try {
      const response = await api.getBigGames()
      if (response.data) {
        setBigGames(response.data)
      } else {
        throw new Error(response.error || "Failed to fetch big games")
      }
    } catch (error) {
      console.error("Error fetching big games:", error)
      setBigGamesError("Unable to load featured games. Please try again later.")
    }
  }
  ...
  const filteredMatches = matches.filter((match) => {
    ...
    const matchesSearch =
      match.home_team.toLowerCase().includes(search) || match.away_team.toLowerCase().includes(search)
    const matchesStatus = statusFilter === "all" || match.status === statusFilter
    return matchesSearch && matchesStatus && match.status !== "fulltime"
  })
        ...
        <main className="flex-1 min-h-screen lg:ml-0">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 pt-20 lg:pt-6">
            ...
            <div className="text-center mb-8 sm:mb-12">
              ...
              {/* Big Games Carousel */}
              <div className="big-games-carousel mb-8 sm:mb-12">
                <Card className="bg-white/5 backdrop-blur-sm border border-white/10 hover:border-white/20 rounded-xl sm:rounded-2xl overflow-hidden">
                  <CardHeader>
                  ...
                  <CardContent className="p-4 sm:p-6">
                    {bigGamesError ? (
                      <p className="text-red-400 text-center text-sm sm:text-base">{bigGamesError}</p>
                    ) : bigGames.length > 0 ? (
                      <Swiper
                        spaceBetween={10}
                        slidesPerView={1}
                          ...
                          <SwiperSlide key={game.id}>
                            <div className="relative">
                              ...
                              <div className="absolute bottom-0 left-0 right-0 bg-black/60 backdrop-blur-sm p-2 sm:p-3">
                                <p className="text-white text-sm sm:text-base font-semibold text-center">{game.title}</p>
                              </div>
                            </div>
                          </SwiperSlide>
                        ))}
                      ...
                      <p className="text-gray-400 text-center text-sm sm:text-base">No featured games available.</p>
                    )}
                  </CardContent>
                </Card>
              </div>
                          ...
                          >
                            <div className="flex items-center justify-between mb-3 sm:mb-4 relative z-10">
                              ...
                              <div className="text-right">
                                ...
                                <div className="text-gray-400 text-xs sm:text-sm">
                                  Odds: {formatOdds(bet.total_odds)}
                                </div>
                                {bet.status === "pending" && (
                                  <div className="text-yellow-400 text-xs sm:text-sm mt-1 font-semibold">
                                    Expected: KES {formatCurrency(Number(bet.amount) * Number(bet.total_odds))}
