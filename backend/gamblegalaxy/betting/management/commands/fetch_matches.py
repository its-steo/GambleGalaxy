...
class Command(BaseCommand):
    help = "Fetches match data and odds from Football API for the upcoming week and updates matches in the database. Drops finished matches before fetching new ones."
    ...
    def handle(self, *args, **kwargs):
        ...
        # Determine the date range for the upcoming week (next 7 days from today)
        today = datetime.now(pytz.UTC).date()
        from_date = today.strftime('%Y-%m-%d')
        to_date = (today + timedelta(days=7)).strftime('%Y-%m-%d')
        self.stdout.write(f"Fetching fixtures from {from_date} to {to_date}")

        # Drop all previous finished matches before fetching new ones
        self.drop_finished_matches()
    ...
    def drop_finished_matches(self):
        """Delete all finished matches from the database."""
        finished_matches = Match.objects.filter(status='fulltime')
        count = finished_matches.count()
        if count > 0:
            finished_matches.delete()
            self.stdout.write(self.style.SUCCESS(f"Dropped {count} finished matches from the database."))
            logger.info(f"Dropped {count} finished matches from the database.")
        else:
            self.stdout.write("No finished matches to drop.")
            logger.info("No finished matches to drop.")
