...
class TransactionSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        logger.info(f"Validating transaction data: {data}, context: {self.context}")
        # Get transaction_type from context (set by view) or raise error if not set
        tx_type = self.context.get('transaction_type')
        if not tx_type:
            logger.error("No transaction_type provided in context")
            raise serializers.ValidationError("Transaction type must be provided by the view.")
        logger.info(f"Determined transaction_type: {tx_type}")

        # Ensure transaction_type in data matches context, if provided
        if data.get('transaction_type') and data['transaction_type'] != tx_type:
            logger.error(f"Transaction type mismatch: data={data['transaction_type']}, context={tx_type}")
            raise serializers.ValidationError(f"Transaction type in data ({data['transaction_type']}) does not match expected type ({tx_type}).")

        if tx_type == 'deposit':
            deposit_method = self.context['request'].data.get('deposit_method', 'stk_push')
            logger.info(f"Deposit method: {deposit_method}")
            if deposit_method == 'manual':
                mpesa_code = self.context['request'].data.get('mpesa_code')
                if not mpesa_code:
                    logger.error("Missing M-Pesa code for manual deposit")
                    raise serializers.ValidationError("M-Pesa code is required for manual deposits.")
                data['mpesa_code'] = mpesa_code
                data['description'] = 'pending: Manual deposit - awaiting system verification'
            elif deposit_method == 'stk_push':
                phone_number = self.context['request'].data.get('phone_number')
                if not phone_number or not phone_number.startswith('254'):
                    logger.error(f"Invalid phone number for STK Push deposit: {phone_number}")
                    raise serializers.ValidationError("Valid phone number starting with '254' is required for STK Push deposit.")
                data['description'] = 'pending: STK initiated - awaiting for system confirmation'
                data['mpesa_code'] = None  # Explicitly set to None for STK Push deposits
            else:
                logger.error(f"Invalid deposit method: {deposit_method}")
                raise serializers.ValidationError("Deposit method must be 'manual' or 'stk_push'.")
        elif tx_type == 'withdraw':
            account_details = self.context['request'].data.get('phoneNumber') or self.context['request'].data.get('accountNumber')
            withdrawal_method = self.context['request'].data.get('withdrawalMethod')
            logger.info(f"Withdrawal method: {withdrawal_method}, account_details: {account_details}")
            if not account_details:
                logger.error("Missing account details for withdrawal")
                raise serializers.ValidationError("Phone number or account number is required for withdrawal.")
            if withdrawal_method == 'mpesa' and not account_details.startswith('254'):
            ...
            if not withdrawal_method:
                logger.error("Missing withdrawal method")
                raise serializers.ValidationError("Withdrawal method must be specified.")
            data['account_details'] = account_details
            data['description'] = 'pending: Awaiting system approval'
            data['mpesa_code'] = None  # Explicitly set to None for withdrawals
    ...
    def create(self, validated_data):
        user = self.context['request'].user
        amount = validated_data['amount']
        tx_type = validated_data.get('transaction_type', self.context.get('transaction_type'))
        description = validated_data.get('description', '')
        payment_transaction_id = validated_data.get('payment_transaction_id', '')
        account_details = validated_data.get('account_details', '')
        mpesa_code = validated_data.get('mpesa_code')  # Use None directly, no default empty string

        logger.info(f"Creating transaction for user {user.id}: tx_type={tx_type}, mpesa_code={mpesa_code}")
