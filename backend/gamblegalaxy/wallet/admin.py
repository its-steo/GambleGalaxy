...
class TransactionAdmin(admin.ModelAdmin):
    ...
    def mark_as_failed(self, request, queryset):
        for transaction in queryset.filter(description__startswith='pending'):
            if transaction.transaction_type == 'withdraw':
                wallet = transaction.user.wallet
                wallet.deposit(transaction.amount)  # Refund the balance
                transaction.description = 'failed: System rejected your withdrawal'
                transaction.save()
                logger.info(f"Withdrawal failed for user {transaction.user.id}, refunded {transaction.amount} to wallet")
            elif transaction.transaction_type == 'deposit':
                transaction.description = 'failed: System rejected your deposit'
                transaction.save()
