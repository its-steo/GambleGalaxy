...
class WalletView(generics.RetrieveAPIView):
    ...
    def get_object(self):
        wallet, created = Wallet.objects.get_or_create(user=self.request.user)
        logger.info(f"Retrieved wallet for user {self.request.user.id}, created={created}")
        return wallet
...
class DepositView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        logger.info(f"DepositView received request: {request.data}, endpoint=/api/wallet/deposit/")
        serializer = self.get_serializer(data=request.data)
        ...
        if deposit_method == 'stk_push':
            if not phone_number or not phone_number.startswith('254'):
                ...
                raise ValidationError("Valid phone number starting with '254' is required for STK Push deposit.")
            ...
            try:
                ...
                if response.get('ResponseCode') != '0':
                    error_msg = response.get('error', 'Failed to initiate STK Push.')
                    logger.error(f"STK Push initiation failed for user {user.id}: {error_msg}")
                    raise ValidationError(f"Failed to initiate STK Push deposit: {error_msg}")

                checkout_request_id = response.get('CheckoutRequestID')
                if not checkout_request_id:
                    logger.error(f"No CheckoutRequestID in response for user {user.id}: {response}")
                    raise ValidationError("Failed to get CheckoutRequestID from payment service for deposit.")
                ...
                transaction = serializer.save(
                    ...
                    transaction_type='deposit',
                    payment_transaction_id=checkout_request_id,
                    description='pending: STK initiated - awaiting for system confrirmation'
                )
                logger.info(f"STK Push transaction saved for user {user.id}: ID={transaction.id}, CheckoutRequestID={checkout_request_id}")
                return Response({
                    'message': 'STK Push initiated. Please complete the payment on your phone.',
                ...
                logger.error(f"Error in DepositView for user {user.id}: {str(e)}")
                raise ValidationError(f"Failed to process STK Push deposit: {str(e)}")
            ...
            transaction = serializer.save(
                ...
                payment_transaction_id=transaction_id,
                mpesa_code=mpesa_code,
                description='pending: Manual deposit - awaiting system verification'
            )
            logger.info(f"Manual deposit transaction saved for user {user.id}: ID={transaction.id}, M-Pesa Code={mpesa_code}")
            return Response({
                'message': 'Manual deposit request submitted with M-Pesa code. Awaiting system approval.',
                'transaction_id': transaction_id
            }, status=status.HTTP_202_ACCEPTED)
...
class WithdrawView(generics.CreateAPIView):
    ...
    def create(self, request, *args, **kwargs):
        logger.info(f"WithdrawView received request: {request.data}, endpoint=/api/wallet/withdraw/")
        serializer = self.get_serializer(data=request.data)
        ...
        logger.info(f"Processing withdrawal: user={user.id}, amount={amount}, withdrawal_method={withdrawal_method}, account_details={account_details}")
        ...
        if wallet.balance < amount:
            logger.error(f"Insufficient balance for user {user.id}: requested={amount}, available={wallet.balance}")
            raise ValidationError("Insufficient balance for withdrawal.")

        with db_transaction.atomic():
            # Balance is updated immediately via serializer
            transaction = serializer.save(
                user=user,
                transaction_type='withdraw',
                payment_transaction_id=f"TXN-{uuid.uuid4().hex[:10]}",
                account_details=account_details,
                description='pending: Awaiting admin approval'
            )

        logger.info(f"Withdrawal request created for user {user.id}: transaction_id={transaction.payment_transaction_id}, balance_updated={wallet.balance}")
        return Response({
            'message': 'Withdrawal request submitted. Awaiting admin approval.',
            'transaction_id': transaction.payment_transaction_id
        }, status=status.HTTP_202_ACCEPTED)
...
class TransactionHistoryView(generics.ListAPIView):
    ...
    def get_queryset(self):
        logger.info(f"Fetching transaction history for user {self.request.user.id}")
        return Transaction.objects.filter(user=self.request.user).order_by('-timestamp')
...
class CallbackView(generics.GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        try:
            data = request.data.get('Body', {}).get('stkCallback', {})
            logger.info(f"CallbackView received data: {data}")
            ...
            with db_transaction.atomic():
                ...
                if result_code == '0':
                    ...
                    transaction.description = f'pending: STK success ({result_desc}) - awaiting system verification'
                    transaction.payment_transaction_id = payment_receipt
                    transaction.account_details = account_details
                    transaction.save()
                    logger.info(f"Callback updated transaction {transaction.id}: Receipt={payment_receipt}, Status=pending")
                    ...
                    try:
                        ...
                        logger.error(f"Failed to send admin notification for {payment_receipt}: {str(e)}")
