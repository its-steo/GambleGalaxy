...
class PaymentClient:
    def __init__(self, callback_url=None):
        ...
        self.stk_push_url = 'https://api.safaricom.co.ke/mpesa/stkpush/v1/processrequest'
        self.query_url = 'https://api.safaricom.co.ke/mpesa/stkpushquery/v1/query'
        logger.debug(f"PaymentClient initialized with stk_push_url: {self.stk_push_url}")
    ...
    def get_access_token(self):
        ...
        try:
            auth = base64.b64encode(f"{self.consumer_key}:{self.consumer_secret}".encode()).decode()
            headers = {'Authorization': f'Basic {auth}'}
            logger.debug(f"Auth request: URL={self.auth_url}")
            response = requests.get(self.auth_url, headers=headers, timeout=10)
            logger.debug(f"Auth response: Status={response.status_code}, Body={response.text}")
            response.raise_for_status()
            access_token = response.json()['access_token']
    ...
    def initiate_stk_push(self, phone_number, amount, transaction_id):
        ...
        try:
            ...
            logger.debug(f"Initiating STK Push: URL={self.stk_push_url}, Payload={payload}")
            response = requests.post(self.stk_push_url, json=payload, headers=headers, timeout=30)
            logger.debug(f"STK Push response: Status={response.status_code}, Body={response.text}")
            response.raise_for_status()
            response_json = response.json()
            if response_json.get('ResponseCode') != '0':
                logger.error(f"STK Push failed: {response_json.get('ResponseDescription', 'Unknown error')}")
                return {'ResponseCode': '1', 'error': response_json.get('ResponseDescription', 'Failed to initiate STK Push')}
            logger.info(f"STK Push success: {response_json}")
            return response_json
        except requests.RequestException as e:
            error_msg = f"STK Push failed: {str(e)}, Response={getattr(e.response, 'text', '')}"
