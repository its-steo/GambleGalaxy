...
class AviatorConsumer(AsyncWebsocketConsumer):
    ...
    async def start_round(self):
        ...
        aviator_round = await sync_to_async(AviatorRound.objects.create)(
    ...
    async def send_multiplier(self, event):
        ...
        while multiplier < crash:
            bot_bets = await sync_to_async(list)(
    ...
    async def manual_cashout(self, user_id, multiplier):
        ...
        try:
            ...
            bet = await database_sync_to_async(
                lambda: AviatorBet.objects.filter(user=user, cashed_out_at__isnull=True).latest("created_at")
            )()
