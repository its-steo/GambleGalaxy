...
#from .models import AviatorRound, AviatorBet, SureOdd, CrashMultiplierSetting, PredictorPurchase, PredictorPackage
#from .serializers import PredictorPackageSerializer
...
#        await self.send_user_predictors()
#
...
#        }))
#
#    async def send_user_predictors(self):
#        """Send user's active predictor purchases and all available predictor packages"""
#        user = self.scope['user']
#        
#        # Use PredictorPackageSerializer to serialize packages
#        packages = await database_sync_to_async(list)(PredictorPackage.objects.all())
#        serializer = PredictorPackageSerializer(packages, many=True, context={
#            'request': None  # Pass None since we don't have a request object
#        })
#        packages_data = serializer.data
#        
#        if not user.is_authenticated:
#            await self.send(json.dumps({
#                "type": "user_predictors",
#                "packages": packages_data,
#                "purchases": []
#            }))
#            return
#
#        purchases = await database_sync_to_async(list)(
#            PredictorPurchase.objects.filter(user=user, expiry_date__gte=timezone.now())
#            .select_related('predictor_package')
#            .values('id', 'predictor_package__name', 'predictor_package__predictions_per_day', 'predictions_used_today')
#        )
#        await self.send(json.dumps({
#            "type": "user_predictors",
#            "packages": packages_data,
...
#                {
#                    "purchase_id": p['id'],
#                    "name": p['predictor_package__name'],
#                    "predictions_remaining": p['predictor_package__predictions_per_day'] - p['predictions_used_today']
#                } for p in purchases
#            ]
#        }))
#
...
#                await self.handle_generate_prediction(data)
#            elif action == "ping":
...
#
#    async def handle_generate_prediction(self, data):
#        user = self.scope['user']
#        if not user.is_authenticated:
#            await self.send(json.dumps({"error": "Authentication required"}))
#            return
...
#        if not purchase_id:
#            await self.send(json.dumps({"error": "Purchase ID required"}))
#            return
#
#        try:
#            purchase = await database_sync_to_async(PredictorPurchase.objects.get)(
#                user=user, id=purchase_id, expiry_date__gte=timezone.now()
#            )
#        except PredictorPurchase.DoesNotExist:
#            await self.send(json.dumps({"error": "Active predictor purchase not found"}))
#            return
#
#        if not await database_sync_to_async(purchase.can_predict)():
#            await self.send(json.dumps({"error": "Daily prediction limit reached or package expired"}))
#            return
#
#        settings = await self.get_crash_multiplier_settings()
#        if settings:
...
#
#        if user.is_nesty:
#            await database_sync_to_async(SureOdd.objects.create)(
#                user=user,
#                odd=prediction,
...
#        await database_sync_to_async(purchase.save)()
#
#        await self.send(json.dumps({
...
class AviatorConsumer(AsyncWebsocketConsumer):
    ...
    async def handle_cashout(self, data):
        ...
        if bet.round.id != current_state['round_id']:
            ...
            return
        try:
            requested_mult = Decimal(str(multiplier))
        except (InvalidOperation, ValueError, TypeError):
            await self.send(json.dumps({"error": "Invalid multiplier", "request_id": request_id}))
            return
        if requested_mult < Decimal('1.00'):
            await self.send(json.dumps({"error": "Invalid multiplier", "request_id": request_id}))
            return
        if bet.cash_out_multiplier is not None:
            await self.send(json.dumps({"error": "Already cashed out", "request_id": request_id}))
            return

        tolerance = Decimal('2.00')
        crash_mult = Decimal(str(current_state['crash_multiplier']))
        current_mult = Decimal(str(current_state['current_multiplier']))
        use_mult = requested_mult

        if current_state['crashed']:
            server_time = int(time.time() * 1000)
            if server_time - current_state['crash_time'] > 3000:  # 3 second grace period
                await self.send(json.dumps({"error": "Too late! Plane crashed", "request_id": request_id}))
                return
            if requested_mult >= crash_mult:
                await self.send(json.dumps({"error": "Too late! Plane crashed", "request_id": request_id}))
                return
            # Allow at requested_mult
        else:
            if requested_mult > current_mult + tolerance:
                await self.send(json.dumps({"error": "Invalid multiplier - too high", "request_id": request_id}))
                return
            # Use the minimum to be safe
            use_mult = min(requested_mult, current_mult)

        win_amount = (bet.amount * use_mult).quantize(Decimal('0.01'))
        bet.cash_out_multiplier = float(use_mult)
        bet.final_multiplier = float(use_mult)
        bet.is_winner = True
        await database_sync_to_async(bet.save)()
        wallet = await self.deposit_wallet(user, win_amount)
        try:
            await database_sync_to_async(Transaction.objects.create)(
                ...
                amount=win_amount,
                transaction_type='winning',
                description=f'Cashed out from Aviator at {use_mult}x'
            )
        except Exception as e:
            await self.send(json.dumps({"error": f"Failed to create transaction: {str(e)}", "request_id": request_id}))
            return
    ...
    async def auto_cashout(self, current_multiplier, aviator_round):
        ...
        def process_batch():
            with transaction.atomic():
                for bet in bets:
                    use_mult = Decimal(str(bet.auto_cashout))
                    win_amount = (bet.amount * use_mult).quantize(Decimal('0.01'))
                    bet.cash_out_multiplier = bet.auto_cashout
                    bet.final_multiplier = bet.auto_cashout
                    bet.is_winner = True
